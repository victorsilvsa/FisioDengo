...
// Definitions of the 25 updated questions
const updates = {
  // World 1
  w1_q11: {
    options: [
...
const fileMap = {
  w1: 'world1_celular.js',
  w2: 'world2_sangue.js',
  w3: 'world3_cardio.js',
  w4: 'world4_respiratorio.js',
  w5: 'world5_integrado.js'
};

const fileData = {};
Object.values(fileMap).forEach(f => {
  fileData[f] = fs.readFileSync('js/data/' + f, 'utf8');
});

Object.entries(updates).forEach(([qId, upd]) => {
  const prefix = qId.split('_')[0];
  const filename = fileMap[prefix];
  let content = fileData[filename];

  // Locate the question in file by id: 'qId'
  const idRegex = new RegExp(`(id:\\s*['"]${qId}['"][\\s\\S]*?options:\\s*\\[)([\\s\\S]*?)(\\][\\s\\S]*?correctIndex:\\s*\\d+)`);
  const match = content.match(idRegex);
  if (!match) {
    console.error(`Não encontrou questão ${qId} em ${filename}`);
    return;
  }

  const newOptionsFormatted = upd.options.map(opt => `        "${opt}"`).join(',\n');
  const replacement = `${match[1]}\n${newOptionsFormatted}\n      ],\n      correctIndex: ${upd.correctIndex}`;
  
  // Replace up to correctIndex
  const fullBlockRegex = new RegExp(`(id:\\s*['"]${qId}['"][\\s\\S]*?options:\\s*\\[)[\\s\\S]*?(\\][\\s\\S]*?correctIndex:\\s*\\d+)`);
  content = content.replace(fullBlockRegex, `${match[1]}\n${newOptionsFormatted}\n      ],\n      correctIndex: ${upd.correctIndex}`);

  fileData[filename] = content;
  console.log(`✓ Atualizada questão ${qId} em ${filename}`);
});

Object.entries(fileData).forEach(([f, content]) => {
  fs.writeFileSync('js/data/' + f, content, 'utf8');
  console.log(`Salvo js/data/${f}`);
});
console.log('Balanceamento aplicado com sucesso!');
