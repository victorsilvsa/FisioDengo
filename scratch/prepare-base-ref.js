...
const fileContent = `// Central Academic References and In-Depth Research Topics Database
...
const REFERENCES_DATA = {
  ...
  // Registra um tema garantindo múltiplos índices (tópico exato, ID de fase, minúsculo)
  ...
  openDeepDiveModal(identifier, selectedSubtopicTitle = null, selectedBookTab = 'guyton') {
    ...
    if (!subtopic && topic.researchTopics) {
      // Se o identifier bateu direto com um subtópico
      ...
      \${subtopic ? \`
        <div class="card" style="margin-bottom: 22px; padding: 18px 20px; background: #ffffff; border: 1.5px solid var(--primary-200); border-radius: var(--radius-lg); box-shadow: var(--shadow-sm);">
          ...
          <div style="font-size: 0.8rem; color: var(--slate-500);">
            <strong>Termos de Pesquisa PubMed:</strong> \`\${subtopic.query || subtopic.title}\`
          </div>
        </div>
      \` : ''}
...
// Delegated listeners para todos os cliques em temas e subtópicos
...
console.log('Base references code prepared.');
